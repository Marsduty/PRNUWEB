...
def image_files_in(path: Path) -> list[Path]:
    ...
    return sorted(
        [
            ...
            if item.is_file() and item.suffix in IMAGE_EXTENSIONS and "(1)" not in item.name
        ],
        key=natural_key,
    )


def choose_flat_bucket(flat_root: Path) -> tuple[str, list[Path]]:
    if not flat_root.exists():
        return "-", []

    buckets: list[tuple[str, list[Path]]] = []
    direct_files = image_files_in(flat_root)
    if direct_files:
        buckets.append((".", direct_files))

    for child in sorted([item for item in flat_root.iterdir() if item.is_dir()], key=lambda item: item.name):
        files = image_files_in(child)
        if files:
            buckets.append((child.name, files))

    if not buckets:
        return "-", []

    return max(buckets, key=lambda item: (len(item[1]), item[0] == "0"))
...
def build_import_plans(dataset_root: Path) -> list[DeviceImportPlan]:
    ...
    for device_dir in sorted([item for item in dataset_root.iterdir() if item.is_dir()]):
        ...
        notes = f"Floreview数据集设备{dataset_index}"
        if dataset_index == 22:
            plans.append(
                DeviceImportPlan(
                    ...
                    candidate_count=0,
                    build_files=[],
                    skipped_reason="D22 为 HEIC/MOV 数据，当前流程跳过",
                )
            )
            continue

        bucket_name, bucket_files = choose_flat_bucket(device_dir / "Flat" / "jpeg-h264")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="批量导入 FloreView Flat 参考图并构建设备指纹")
    parser.add_argument("--dataset", default=r"F:\ExpDataset\FloreView\Dataset", help="FloreView Dataset 根目录")
